...
export class SalesOrdersController {
  ...
  updateStatus(@Param('id') id: string, @Body() body: { status: string }, @CurrentUser() user: AuthUser) {
    ...
    return this.salesOrders.updateStatus(id, body.status, user.id);
  }
  ...
  approve(@Param('id') id: string, @CurrentUser() user: AuthUser) {
    return this.salesOrders.approve(id, user.id, user.role);
  }
  ...
  reject(@Param('id') id: string, @Body() body: { reason: string }, @CurrentUser() user: AuthUser) {
    ...
    return this.salesOrders.reject(id, user.id, body.reason.trim(), user.role);
  }
  ...
  createInvoice(@Param('id') id: string, @CurrentUser() user: AuthUser) {
    return this.salesOrders.convertToInvoice(id, user.id);
  }
}
