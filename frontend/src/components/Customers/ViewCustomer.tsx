...
import { getCustomerById, deleteCustomer, createDeal, getDeals, getQuotes, getInvoices, getProjects } from "@/utils/api";
...
const ViewCustomer: React.FC = () => {
  ...
  const canDelete = (user?.permissions ?? []).some((p) => p === '*' || p === 'contacts:delete');
  ...
  const menuItems: DetailMenuItem[] = [
    ...
    { label: "Convert to Deal", icon: <Handshake className="h-3.5 w-3.5 me-2" />, onClick: openConvertDialog },
  ];
  if (canDelete) {
  ...
  const header = (
    <DetailHeader
      crumbs={[{ label: "Customers", href: "/customers" }, { label: customerData.name }]}
      title={customerData.name}
      primaryAction={
        <Button size="sm" className="gap-1" onClick={() => navigate(`/customers/${id}/edit`)}>
  ...
  const contextPanel = id ? (
    <RecordContextPanel linkedTo={id} linkedModel="Customer" />
  ) : undefined;
          ...
          <CardContent className="py-5">
            <Tabs defaultValue="deals">
              <TabsList className="mb-4 flex h-auto w-full justify-start gap-1 overflow-x-auto [&>button]:shrink-0">
                <TabsTrigger value="deals">Deals{dealsCount > 0 && ` (${dealsCount})`}</TabsTrigger>
                <TabsTrigger value="projects">Projects{projectsCount > 0 && ` (${projectsCount})`}</TabsTrigger>
