...
function TaskCard({
  task, onEdit, onDelete, onComplete, onClone, priorities,
}: {
  task: Task;
  onEdit: () => void;
  onDelete: () => void;
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-1.5 mt-2">
            ...
            {(task.tags ?? []).map((tag) => (
              <span key={tag} className="inline-flex px-1.5 py-0.5 rounded-full bg-secondary text-secondary-foreground text-[10px]">
                {tag}
              </span>
            ))}
          </div>
...
export function Tasks() {
  ...
  const { data, isPending } = useQuery({
    queryKey: ["tasks", filter, projectFilter],
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <Button size="sm" className="gap-1.5 shrink-0" onClick={() => setPanelOpen(true)}>
          <PlusCircle className="h-3.5 w-3.5" />{t.add}
        </Button>
      </div>
